...
public class CommonReward implements Agent {
    ...
    private class TaskModel {
        ...
        // A buffer to hold the sum of the intent and common rewards
        double[] buffer;
        ...
        TaskModel(String name) {
            ...
            // Construct reward buffer
            buffer = new double[rewards.intentSize()];
        ...
        void update() {
            ...
            for(int sample = 0; sample < intent.numSamples(); ++sample) {
                ...
                double[] intent_sample = intent.value();
                double[] common_sample = common.value();

                // Combine intent and common reward and set as the planning intent
                for(int i=0; i < rewards.intentSize(); ++i)
                    buffer[i] = intent_sample[i] + common_sample[i];

                graph.setIntent(buffer);
        ...
        void updatePolicy() {
            ...
            double[] intent_mean = intent.mean();
            double[] common_mean = common.mean();

            for(int i=0; i < rewards.intentSize(); ++i)
                buffer[i] = intent_mean[i] + common_mean[i];
    ...
    public List<Visualization> visualizations() {
        ...
        for(Map.Entry<String, TaskModel> task : tasks.entrySet()) {
            ...
            double[] intent_mean = model.intent.mean();
            double[] common_mean = common.mean();

            for(int i=0; i < rewards.intentSize(); ++i)
                model.buffer[i] = intent_mean[i] + common_mean[i];

            rewards.render(model.buffer)
